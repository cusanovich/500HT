...
pdf("/mnt/lustre/home/cusanovich/500HT/Imputed1415/tissues.pdf")
for(i in 1:length(gwass)){
	...
	plot(-log10(as.numeric(colnames(currenrichments))),currenrichments[1,],type="l",ylim=c(0,max(currenrichments)),main=currpheno)
	for(i in 2:dim(currenrichments)[1]){
		lines(-log10(as.numeric(colnames(currenrichments))),currenrichments[i,])
	}
}
dev.off()
