...
**Square matrix of relatedness** can either be generated by GEMMA itself, or by extraction from the additive covariance matrix from the Abney lab.  
   ...
   - If you want to use the additive kinship matrix from the Abney lab make sure to specify "-km 2" when running GEMMA. This will pull out the necessary pairs based on findivs.  
...
This genotype data can then be converted into a set of tabix-indexed files (separate files for each chromosome) with cis/plink_bed2tabix.py.
    ...
    Note: Submit the command 'python plink_bed2tabix.py' on the node and it will submit the jobs automatically. You must be able to submit jobs from node.
