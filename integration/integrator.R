...
lister = c(1:1000)
pdf("/mnt/lustre/home/cusanovich/extreme_pval_enrichments.pdf")
for(i in 1:length(gwass)){
	...
	commongenes = match(intersect(exprs[,2],eqtls[,1]),eqtls[,1])
	commonsnps = match(intersect(gwas[,1],eqtls[,2]),eqtls[,2])
	curreqtls = eqtls[intersect(commongenes,commonsnps),]
