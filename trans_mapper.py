...
pfile = open('../ByChr/chr' + chrm + '.trans.pvals.txt','w')
sigfile = open('../ByChr/chr' + chrm + '.trans.sig.txt','w')
for gene in range(columncount):
#for gene in range(2):
	gemmer = ('~/Programs/gemma0.94 -bfile ./chr' + chrm + ' -km 2 -k ../addSNP.500ht.txt -lmm 2 -n ' + str(gene + 1) + ' -maf 0.05 -o chr' + chrm)
	ifier(gemmer)
	assoc = matrix_reader(genodir + 'output/chr' + chrm + '.assoc.txt')
	assoc = assoc[1:,:]
...
pfile.close()
sigfile.close()
cleanup = 'rm ./chr' + chrm + '.fam; rm ./chr' + chrm + '.bed; rm ./chr' + chrm + '.bim'
ifier(cleanup)
